...
BEGIN
	...
	SELECT @Score=Score,@InsureScore=InsureScore FROM THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo WHERE UserID=@dwUserID
	IF @@rowcount = 0
	BEGIN
		-- 插入资料
	...
	-- 领取金币	
	UPDATE THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo SET Score = Score + @ScoreAmount WHERE UserID = @dwUserID
