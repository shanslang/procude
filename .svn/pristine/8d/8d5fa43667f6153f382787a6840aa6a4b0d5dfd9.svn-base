...
BEGIN	
	...
	SELECT @IsAndroidUser=IsAndroid,@UserMemberOrder=MemberOrder FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	BEGIN
		...
		BEGIN	
			...
			BEGIN
				...
				-- 更新金币
				UPDATE THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo SET Score=@Score-@lMatchFee WHERE UserID=@dwUserID				
			END
			...
			BEGIN
				...
				-- 更新元宝
				UPDATE THAccountsDBLink.THAccountsDB.dbo.AccountsInfo SET UserMedal=@wUserMedal-@lMatchFee WHERE UserID=@dwUserID	
			END
		END
	END
	...
	SELECT @CurrScore=Score FROM THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo WHERE UserID=@dwUserID		
	
	-- 查询元宝
	DECLARE @CurrIngot BIGINT	
	SELECT @CurrIngot=UserMedal FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
...
BEGIN
	...
	-- 用户信息
	DECLARE @IsAndroidUser TINYINT	
	SELECT @IsAndroidUser=IsAndroid FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	BEGIN
		...
		BEGIN
			...
			-- 更新元宝
			UPDATE THAccountsDBLink.THAccountsDB.dbo.AccountsInfo SET UserMedal=@UserMedal+@lMatchFee WHERE UserID=@dwUserID	
		END	
	END
