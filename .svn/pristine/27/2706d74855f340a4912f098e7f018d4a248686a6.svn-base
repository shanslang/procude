...
--	END
	...
	SELECT @Insure=InsureScore FROM THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo WHERE UserID=@dwUserID
	...
	-- 设置练习币
	DECLARE @EducateGrantScore AS INT
	...
	IF @EducateGrantScore IS NULL 
	BEGIN
		SET @EducateGrantScore=0
	END

	SET @Score = @EducateGrantScore
