...
BEGIN
	...
	FROM THPlatformDBLink.THPlatformDB.dbo.GlobalPlayPresent WHERE ServerID=@wServerID
	IF @ServerID IS NULL
	BEGIN
		SELECT @ServerID=ServerID,@PresentMember=PresentMember,@CellPlayPresnet=CellPlayPresnet,@CellPlayTime=CellPlayTime,@StartPlayTime=StartPlayTime,
		@CellOnlinePresent=CellOnlinePresent,@CellOnlineTime=CellOnlineTime,@StartOnlineTime=StartOnlineTime,
		@IsPlayPresent=IsPlayPresent,@IsOnlinePresent=IsOnlinePresent
		FROM THPlatformDBLink.THPlatformDB.dbo.GlobalPlayPresent WHERE ServerID=-2
		IF @ServerID IS NULL
		BEGIN
			SET @IsPlayPresent=0
	...
	FROM THPlatformDBLink.THPlatformDB.dbo.GlobalPlayPresent WHERE ServerID=-3
	...
	BEGIN
		...
		SELECT @UserDatePresent=PresentScore FROM THTreasureDBLink.THTreasureDB.dbo.StreamPlayPresent WHERE DateID=@DateID AND UserID=@dwUserID
	...
	BEGIN
		...
		SELECT @UserPresent=SUM(PresentScore) FROM THTreasureDBLink.THTreasureDB.dbo.StreamPlayPresent WHERE UserID=@dwUserID
    ...
    SELECT @MemberOrder=MemberOrder FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	BEGIN		
		-- 更新用户金币
		UPDATE THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo set Score=Score+@PresentScore WHERE UserID=@dwUserID

		UPDATE THTreasureDBLink.THTreasureDB.dbo.StreamPlayPresent SET PresentCount=PresentCount+1,PresentScore=PresentScore+@PresentScore,
		...
		IF @@ROWCOUNT=0
		BEGIN
			INSERT THTreasureDBLink.THTreasureDB.dbo.StreamPlayPresent (DateID,UserID,PresentCount,PresentScore,PlayTimeCount,OnLineTimeCount)
