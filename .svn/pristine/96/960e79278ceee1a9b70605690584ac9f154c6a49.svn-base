...
BEGIN
	...
	IF @cbByNickName=1
		SELECT @TargetGameID=GameID, @Accounts=NickName FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE NickName=@strAccounts
	ELSE
		SELECT @TargetGameID=GameID, @Accounts=NickName FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE GameID=@strAccounts
