...
BEGIN
	...
	SELECT @UserMemberOrder=MemberOrder,@LogonPassword=LogonPass FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	FROM THGameMatchDBLink.THGameMatchDB.dbo.MatchPublic AS a,THGameMatchDBLink.THGameMatchDB.dbo.MatchLockTime AS b 
	...
	-- 重复报名
	IF Exists(SELECT * FROM THGameMatchDBLink.THGameMatchDB.dbo.StreamMatchFeeInfo 
	...
	BEGIN
		...
		-- 更新金币
		UPDATE THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo SET Score=@Score-@dwMatchFee WHERE UserID=@dwUserID				
	END
	...
	BEGIN
		...
		SELECT @wUserMedal=UserMedal FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
		...
		-- 更新元宝
		UPDATE THAccountsDBLink.THAccountsDB.dbo.AccountsInfo SET UserMedal=@wUserMedal-@dwMatchFee WHERE UserID=@dwUserID	
	END

	-- 插入记录	
	INSERT THGameMatchDBLink.THGameMatchDB.dbo.StreamMatchFeeInfo (UserID,ServerID,MatchID,MatchNo,MatchFeeType,MatchFee,CollectDate) 
			VALUES(@dwUserID,@wServerID,@dwMatchID,@dwMatchNO,@cbMatchFeeType,@dwMatchFee,GETDATE())

	-- 更新分数
	IF Exists(SELECT * FROM THGameMatchDBLink.THGameMatchDB.dbo.MatchScoreInfo 
	WHERE UserID=@dwUserID AND ServerID=@wServerID AND MatchID=@dwMatchID AND MatchNo=@dwMatchNo)
	BEGIN
		-- 更新数据
		UPDATE THGameMatchDBLink.THGameMatchDB.dbo.MatchScoreInfo 
		SET Score=@dwMatchInitScore,WinCount=0,LostCount=0,DrawCount=0,FleeCount=0
		WHERE UserID=@dwUserID AND ServerID=@wServerID AND MatchID=@dwMatchID AND MatchNo=@dwMatchNo						
	END ELSE						
	BEGIN	
		-- 插入资料
		INSERT INTO THGameMatchDBLink.THGameMatchDB.dbo.MatchScoreInfo(UserID,ServerID,MatchID,MatchNo,Score)
...
BEGIN		
	...
	-- 查询用户
	DECLARE @LogonPassword NCHAR(32) 
	SELECT @LogonPassword=LogonPass FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	IF @LogonPassword IS NULL
	BEGIN
		SET @strErrorDescribe = N'您的用户信息不存在,请与我们的客服人员联系！'
		return 1				
	...
	DECLARE @dwMatchFee BIGINT
	DECLARE @cbMatchFeeType SMALLINT 	
	SELECT TOP 1 @MatchNo=MatchNo,@cbMatchFeeType=MatchFeeType,@dwMatchFee=MatchFee FROM THGameMatchDBLink.THGameMatchDB.dbo.StreamMatchFeeInfo 
	WHERE UserID=@dwUserID AND ServerID=@wServerID AND MatchID=@dwMatchID AND Effective=0
	...
	DECLARE @MatchEndTime datetime	
	DECLARE	@MatchStartTime datetime	
	SELECT @MatchStartTime=StartTime,@MatchEndTime=EndTime FROM THGameMatchDBLink.THGameMatchDB.dbo.MatchLockTime 
	...
	BEGIN
		...
		SELECT @PlayTotalCount=WinCount+LostCount+DrawCount+FleeCount FROM THGameMatchDBLink.THGameMatchDB.dbo.MatchScoreInfo
		WHERE UserID=@dwUserID AND ServerID=@wServerID AND MatchID=@dwMatchID AND MatchNo=@dwMatchNo
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @UserMedal=UserMedal FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
			...
			-- 更新元宝
			UPDATE THAccountsDBLink.THAccountsDB.dbo.AccountsInfo SET UserMedal=@UserMedal+@dwMatchFee WHERE UserID=@dwUserID	
		END			
	END

	-- 删除记录
	DELETE THGameMatchDBLink.THGameMatchDB.dbo.StreamMatchFeeInfo 
