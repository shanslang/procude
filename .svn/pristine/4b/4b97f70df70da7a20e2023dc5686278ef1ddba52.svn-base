...
BEGIN
	...
	IF @dwRewardGold>0
	BEGIN
		UPDATE THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo SET Score=Score+@dwRewardGold WHERE UserID=@dwUserID
		IF @@ROWCOUNT=0
		BEGIN
			INSERT THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo (UserID,Score,LastLogonIP) 
	...
	BEGIN
		UPDATE THAccountsDBLink.THAccountsDB.dbo.AccountsInfo SET UserMedal=UserMedal+@dwRewardIngot,Experience=Experience+@dwRewardExperience 
	...
	SELECT @CurrGold=Score FROM THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo WHERE UserID=@dwUserID

	-- 查询元宝和经验
	DECLARE @CurrIngot INT
	DECLARE @CurrExperience INT	
	SELECT @CurrIngot=UserMedal,@CurrExperience=Experience FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
