...
BEGIN
	...
	SELECT @Nullity=Nullity, @CurrMemberOrder=MemberOrder FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwSourceUserID
	...
	-- 目标用户
	IF @dwTargetUserID<>0 AND NOT EXISTS( SELECT UserID FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwTargetUserID )
	BEGIN
	...
	BEGIN
		...
		SELECT @Insure=InsureScore FROM THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo WHERE UserID=@dwSourceUserID
		...
		-- 银行扣费
		UPDATE THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo SET InsureScore=InsureScore-@ConsumeGold WHERE UserID=@dwSourceUserID	
	END
	ELSE
	BEGIN
	...
	BEGIN
		...
		FROM THAccountsDBLink.THAccountsDB.dbo.AccountsMember WHERE UserID=@dwTargetUserID
	...
	-- 会员等级
	SELECT @CurrMemberOrder=MemberOrder FROM THAccountsDBLink.THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwTargetUserID

	-- 购买记录
	IF @wPropertyID<=13 -- 礼物
	BEGIN
		INSERT THRecordDBLink.THRecordDB.dbo.RecordSendPresent(PresentID,RcvUserID,SendUserID,LovelinessRcv,LovelinessSend,PresentPrice,PresentCount,KindID,ServerID,SendTime,ClientIP)
	...
	ELSE
	BEGIN -- 道具
		INSERT THRecordDBLink.THRecordDB.dbo.RecordSendProperty(PropID,SourceUserID,TargetUserID,PropPrice,PropCount,KindID,ServerID,SendTime,ClientIP)
