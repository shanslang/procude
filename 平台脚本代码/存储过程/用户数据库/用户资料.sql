...
BEGIN
	...
	-- 效验昵称
	DECLARE @strCheckName VARCHAR(32)
	SET @strCheckName = LTRIM(RTRIM(@strNickName))
	IF (DATALENGTH(@strCheckName)<4 OR DATALENGTH(@strCheckName)>16)
	BEGIN
		SET @strErrorDescribe=N'您所输入的游戏昵称长度必须4-16个字符，请更换昵称名后再次修改！'
		RETURN 4
	END

	-- 效验昵称
	IF PATINDEX('%[^吖-座A-Za-z0-9_]%', @strNickName)!=0
	BEGIN
		SET @strErrorDescribe=N'您所输入的游戏昵称只能包含汉字、字母、数字及下划线，请更换昵称名后再次修改！'
		RETURN 4
	END
	...
	BEGIN
		INSERT INTO THRecordDBLink.THRecordDB.dbo.RecordAccountsExpend(UserID,ReAccounts,ClientIP)
