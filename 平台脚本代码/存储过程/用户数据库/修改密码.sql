...
BEGIN
	...
	-- 修改密码记录
	INSERT INTO THRecordDBLink.THRecordDB.dbo.RecordPasswdExpend(OperMasterID,UserID,ReLogonPasswd,ReInsurePasswd,ClientIP)
...
CREATE PROC GSP_GP_ModifyInsurePassword
	@dwUserID INT,								-- 用户 I D
	@strSrcPassword NCHAR(32),					-- 用户原密码
	@strDesPassword NCHAR(32),					-- 用户新密码
	@strDynPassword NCHAR(32),					-- 动态密码
	@strClientIP NVARCHAR(15),					-- 连接地址
	@strErrorDescribe NVARCHAR(127) OUTPUT		-- 输出信息
WITH ENCRYPTION AS
...
BEGIN
	...
	-- 查询用户
	SELECT @UserID=UserID, @InsurePass=InsurePass, @Nullity=Nullity, @StunDown=StunDown,@DynamicPass=DynamicPass
	FROM AccountsInfo(NOLOCK) WHERE UserID=@dwUserID
	...
	-- 动态密码判断
	IF @DynamicPass <> @strDynPassword
	BEGIN
		SET @strErrorDescribe=N'抱歉，密码错误！'
		RETURN 3
	END
	...
	-- 修改密码
	UPDATE AccountsInfo SET InsurePass=@strDesPassword WHERE UserID=@dwUserID

	-- 修改密码记录
	INSERT INTO THRecordDBLink.THRecordDB.dbo.RecordPasswdExpend(OperMasterID,UserID,ReLogonPasswd,ReInsurePasswd,ClientIP)
	VALUES(0,@dwUserID,N'',@strDesPassword,@strClientIP)
