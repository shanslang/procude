...
BEGIN
	...
	SELECT TOP 1 @NowGrowLevelID=LevelID FROM THPlatformDBLink.THPlatformDB.dbo.GrowLevelConfig
	...
	BEGIN
		...
		-- 查询奖励
		SELECT @RewardGold=SUM(RewardGold),@RewardIngot=SUM(RewardMedal) FROM THPlatformDBLink.THPlatformDB.dbo.GrowLevelConfig
		...
		-- 更新金币
		UPDATE THTreasureDBLink.THTreasureDB.dbo.GameScoreInfo SET Score=Score+@RewardGold WHERE UserID=@dwUserID
		IF @@rowcount = 0
		BEGIN
			-- 插入资料
	...
	SELECT @UpgradeExperience=Experience,@UpgradeRewardGold=RewardGold,@UpgradeRewardMedal=RewardMedal FROM THPlatformDBLink.THPlatformDB.dbo.GrowLevelConfig
