...
BEGIN
	...
	FROM THPlatformDB.dbo.GlobalPlayPresent WHERE ServerID=@wServerID
	IF @ServerID IS NULL
	BEGIN
		SELECT @ServerID=ServerID,@PresentMember=PresentMember,@CellPlayPresnet=CellPlayPresnet,@CellPlayTime=CellPlayTime,@StartPlayTime=StartPlayTime,
		@CellOnlinePresent=CellOnlinePresent,@CellOnlineTime=CellOnlineTime,@StartOnlineTime=StartOnlineTime,
		@IsPlayPresent=IsPlayPresent,@IsOnlinePresent=IsOnlinePresent
		FROM THPlatformDB.dbo.GlobalPlayPresent WHERE ServerID=-1
		IF @ServerID IS NULL
		BEGIN
			SET @IsPlayPresent=0
	...
	FROM THPlatformDB.dbo.GlobalPlayPresent WHERE ServerID=-3
    ...
    SELECT @MemberOrder=MemberOrder FROM THAccountsDB.dbo.AccountsInfo WHERE UserID=@dwUserID
	...
	UPDATE GameScoreInfo SET Score=Score+@lVariationScore+@PresentScore, InsureScore=InsureScore+@lVariationInsure, Revenue=Revenue+@lVariationRevenue,
		...
		--, PlayTimeCount=PlayTimeCount+@dwVariationPlayTimeCount, OnLineTimeCount=OnLineTimeCount+@dwOnLineTimeCount
	...
	declare @mem int
	set @ml = (@dwOnTableTimeCount/300)
	select @IsAndroid=IsAndroid,@mem=[MemberOrder] from [THAccountsDB].[dbo].[AccountsInfo](nolock) where UserID = @dwUserID

	if @ml > 0 and @wKindID <> 104 and @wKindID <> 2009 and @wKindID <> 7 and @wKindID <> 127 and @wKindID <> 383 and @wKindID <> 105 and @IsAndroid = 0 and @mem = 0
	begin 
