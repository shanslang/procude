...
BEGIN
	...
	-- 系统暂停
	SELECT @EnjoinLogon=StatusValue FROM THAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'EnjoinLogon'
	IF @EnjoinLogon IS NOT NULL AND @EnjoinLogon<>0
	BEGIN
		SELECT @strErrorDescribe=StatusString FROM THAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'EnjoinLogon'
		RETURN 2
	END
	...
	declare @IsAndroid as tinyint
	declare @MustChangePassWord tinyint
	SELECT @UserID=UserID, @GameID=GameID, @NickName=NickName, @UnderWrite=UnderWrite, @LogonPass=LogonPass,@FaceID=FaceID, @CustomID=CustomID,@MustChangePassWord=MustChangePassWord,
		...
		@MasterRight=MasterRight, @MasterOrder=MasterOrder, @MemberOrder=MemberOrder, @MoorMachine=MoorMachine, @MachineID=LastLogonMachine,@IsAndroid=IsAndroid
	...
	IF @MachineID<>@strMachineID AND @strMachineID<>''
	BEGIN
		SET @strErrorDescribe=N'令牌重置，请重新登录'+@MachineID+N'__'+@strMachineID
		RETURN 2
	END
	...
	-- 密码判断
	IF @LogonPass<>@strPassword AND @IsAndroid = 0
	BEGIN
		SET @strErrorDescribe=N'抱歉,由于您的帐号在其它地方登录导致验证令牌被重置,请重新登录平台后再次尝试！'
		RETURN 3
	END
	...
	BEGIN
		...
		UPDATE THAccountsDB.dbo.AccountsInfo SET MoorMachine=@MoorMachine, LastLogonMachine=@strMachineID WHERE UserID=@UserID
	END
	...
	BEGIN
		...
		RETURN 14
	END
	--delete from GameScoreLocker where userid = @dwUserID and ServerID = @wServerID

	IF @strClientIP<>N'0.0.0.0'
	begin
		begin try
			INSERT GameScoreLocker (UserID, ServerID, KindID, EnterID, EnterIP, EnterMachine) VALUES (@dwUserID, @wServerID, @wKindID, 0, @strClientIP, @strMachineID)
		end try
		begin catch
			-- 查询锁定
			...
			BEGIN
				...
				BEGIN
					-- 提示消息
					set @strErrorDescribe=N'您正在进行保险柜处理过程中，暂时无法进入此游戏房间！'
					RETURN 4
				END
			END
	...
	update GameScoreLocker set EnterID=@InoutIndex where UserID=@dwUserID
...
BEGIN
	...
	--游戏维护
	if exists (select 1 from [THPlatformDB].[dbo].[SystemStatus] where [id] = 1 and [KindMainTain] = @wKindID)
	begin
		if not exists (select 1 from [THPlatformDB].[dbo].[SystemMaintainUsers] where userid = @dwUserID)
		begin
			SET @strErrorDescribe=N'抱歉地通知您，当前房间目前维护中，请稍后再试！'
			RETURN 100
		end
	end
	...
	-- 固定机器
	IF @MoorMachine<>0 and @MachineID <> @strMachineID
	BEGIN
	...
	BEGIN
		SET @strErrorDescribe=N'您的密码不正确，请联系客户服务中心了解详细情况！'
		RETURN 3
	END
	...
	begin catch
		...
		BEGIN
			...
			END
		END
		SELECT @strErrorDescribe=N'未知错误！'
		return 4
	end catch
